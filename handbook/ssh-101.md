1. First, generate a public/private key for your local machine (client):
```zsh
$ ssh-keygen -t ed25519 -C "100067942@ku.ac.ae"
```
Public key created in `~/.ssh/id_ed25519.pub`

2. Then, provide the remote machine (host) your public key (to establish secure connection):
Add your public key to `~/.ssh/authorized_keys` on the remote host

3. Now you can remotely connect to the host:
```zsh
